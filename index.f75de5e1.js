...
function getCurrentCounts() {
    ...
    const columns = rows.length > 0 ? rows[0].getElementsByTagName("td").length : 0;
    return {
...
const addColumn = ()=>{
    ...
    if (currentColumns < MAX_COUNT) {
        for (const row of field.rows)row.insertCell();
        updateButtonStates();
    }
};
const deleteColumn = ()=>{
    ...
    if (currentColumns > MIN_COUNT) {
        for (const row of field.rows)row.deleteCell(-1);
        updateButtonStates();
    }
};
